...
So, the first to jobs in this file build the webapp and the restapi (in parallel). If everything goes well, check the e2e tests (later in this document) and if these acceptance tests pass ok, start deployment. For that we are going to use [Heroku](heroku.com). Heroku allows as to deploy docker containers. For the database we are going to use [MongoDB Atlas](https://www.mongodb.com/cloud/atlas) which will provide a free mongodb database in the cloud that will be enough for our application.

In order to configure the MongoDB Atlas DB, we need to create an account in this [page](https://www.mongodb.com/cloud/atlas). Once created, we need to create a user to connect to the database. Take note of the password. Next, we need to get the connection string for the database, that will look something like `mongodb+srv://<mongouser>:<password>@<host>`. We need to create a GitHub Secret with this URI, under the name `MONGO_URI`. How to know if your configuration is right? When you make a push to the project, GitHub Actions will try to deploy your app to Heroku. Our webservice in Heroku will try to connect to this database. If the application in Heroku works, great, the configuration is correct. If not (for instance, it doesn't insert a new user in the system from the webapp), its time for checking the Heroku app logs to see what is failing (mostly sure, the MONGO_URI string). 

<mark>In order to deploy to Heroku we need that GitHub actions has access to the Heroku api key. We have to configure this under the GitHub Secrets with the name `HEROKU_API_KEY`.</mark>
